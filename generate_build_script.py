...
def generate_build_args(
    os_name, os_version, python_version, pytorch_version, cuda_version, cuda_flavor=None
):
    if os_version not in OS_VERSIONS[os_name]:
        raise ValueError(
            f"OS {os_name} {os_version} is not available: choose from {OS_VERSIONS[os_name]}!"
        )

    if cuda_version == "cpu":
        base_image = "{}:{}".format(os_name, os_version)
        image_tag = "{}-py{}-{}{}".format(
            pytorch_version, python_version, os_name, os_version
        )
    else:
        if os_version not in CUDA_VERSIONS[cuda_version][os_name + "_available"]:
            raise ValueError(
                f"CUDA {cuda_version} is not available in {os_name} {os_version}!"
            )
        if cuda_flavor is None:
            ...
            image_tag = "{}-py{}-cuda{}-{}{}".format(
                pytorch_version,
                python_version,
                CUDA_VERSIONS[cuda_version]["version_name"],
                os_name,
                os_version,
            )
        else:
            ...
            base_image = "nvidia/cuda:{}-cudnn{}-{}-{}{}".format(
                CUDA_VERSIONS[cuda_version]["version_name"],
                CUDA_VERSIONS[cuda_version]["cudnn"],
                cuda_flavor,
                os_name,
                os_version,
            )
...
def generate_build_sh(
    ...
    pytorch_args, kwargs = generate_build_args(
        os_name, os_version, python_version, pytorch_version, cuda_version, cuda_flavor
    )

    content = BUILD_SH_TEMPLATE[os_name].format(*pytorch_args, **kwargs)

    file_path = os.path.join(
        save_dir, "build_{}.sh".format(kwargs["image_tag"].replace("-", "_"))
    )
...
def generate_github_build_yml(
    ...
    file_path = os.path.join(
        save_dir, "docker_build_{}.yml".format(kwargs["image_tag"].replace("-", "_"))
    )
    with open(file_path, "w") as f:
        f.write(content)

    print(
        "Image '{}' generated, please put the following content in the README.md: ".format(
            kwargs["image_tag"]
        )
    )
    print("=" * 50)
        ...
        README_TEMPLATE.format(
            ...
            (
                ...
                if cuda_version == "cpu"
                    ...
                    "cuda"
                    + cuda_version
                    + ("" if cuda_flavor is None else "-" + cuda_flavor)
                )
            ),
            os_name,
            os_version,
...
def parse_args():
    ...
    parser.add_argument(
        "--cuda", help="CUDA version, `cpu` means CPU version.", default="cpu"
    )
    parser.add_argument(
        "--cuda-flavor",
        help="CUDA flavor, `runtime` or `devel`, default is None, means use base image",
    )
...
if __name__ == "__main__":
    args = parse_args()
    generate_build_sh(
        args.os, args.os_version, args.python, args.pytorch, args.cuda, args.cuda_flavor
    )
    generate_github_build_yml(
        args.os, args.os_version, args.python, args.pytorch, args.cuda, args.cuda_flavor
    )
